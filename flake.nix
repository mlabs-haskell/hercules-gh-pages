  ...
  outputs = { self, nixpkgs, hercules-ci-effects }:
    let
      ...
      mkGhPagesJob = config: herculesEnv: {
        onPush.gh-pages.outputs.effects.default = mkGhPagesEffect config herculesEnv;
      };
      mkGhPagesEffect =
        { gh-pages
        , branchName ? "gh-pages"
    ...
    in
    {
      lib = { inherit mkGhPagesEffect mkGhPagesJob; };
      ...
      herculesCI = mkGhPagesJob { inherit (self.packages.${system}) gh-pages; rewriteHistory = false; };
    };
}
